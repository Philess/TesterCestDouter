...
2. Navigate to the provided URL using the Playwright MCP Server. If no URL is provided, look for a default one in the given workspace.
3. If no specific scenario is provided, identify and interact with 3-5 core features or user flows.
...
Put all generated files in the `./reports` directory. You generate the main report, named `a11y-review-report.md` in this folder and the screenshots should be placed in a subdirectory named `screenshots` to ease their inclusion in the report with a relative path.
